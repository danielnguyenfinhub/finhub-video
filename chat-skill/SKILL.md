...
Code you write targets the `danielnguyenfinhub/finhub-video` repository:
...
- **In claude.ai chat (no filesystem):** you cannot run `npm`, Remotion Studio, or a render. Deliver complete, ready-to-save files (full file contents, with the exact target path such as `src/MyScene.tsx`), never fragments. Remind the user to preview/render in Claude Code, Claude Cowork, or a terminal: `npm run dev` to preview, `npx remotion render` to export.
- **In Claude Cowork / Claude Code (filesystem available):** edit the files directly, run `npm run lint` before finishing, and use `npm run dev` to preview. Prefer the project-local skills in `.claude/skills/` if present — they are the same guides as this bundle.
