...
- **Rendering in the browser (`@remotion/web-renderer`).** A button renders frames 45–104 of the reel (2 s) to a video file in the tab with WebCodecs, no server, and offers it as a download. `canRenderMediaOnWeb()` picks MP4 when the browser can encode it, else WebM. Checked in headless Chromium: a 1280×720 VP8 WebM of 60 frames, 372 KB, in 49 s on a software GPU (that Chromium can't encode H.264, so it chose WebM). The web renderer draws the page's layout itself and supports only some CSS (see Remotion's "Limitations of client-side rendering"): the reel's radial-gradient backgrounds come out as the flat background colour. Every web render sends Remotion a telemetry ping (IP, page domain, video or still, never content); `isProduction: false` marks these as development renders.
